...
class LibvirtGenericVIFDriver(object):
    ...
    def plug_vhostuser_ovs(self, instance, vif):
        ...
        iface_id = self.get_ovs_interfaceid(vif)
        vif_path = vif['details'][network_model.VIF_DETAILS_VHOSTUSER_SOCKET]
        vif_mode = vif['details'][network_model.VIF_DETAILS_VHOSTUSER_MODE]

        port_name = os.path.basename(vif_path)
        mtu = vif['network'].get_meta('mtu')
        interface_type = network_model.OVS_VHOSTUSER_CLENT_INTERFACE_TYPE
        if vif_mode == "client":
            interface_type = network_model.OVS_VHOSTUSER_CLENT_INTERFACE_TYPE
