...
def _create_ovs_vif_cmd(bridge, dev, iface_id, mac,
                        instance_id, interface_type=None, **kws):
    ...
    if interface_type == 'dpdkvhostuserclient':
        path = kws.get('vhostuser_socket')
        cmd += ['options:vhost-server-path=%s' % path]
    return cmd


def create_ovs_vif_port(bridge, dev, iface_id, mac, instance_id,
                        mtu=None, interface_type=None, **kws):
    ...
    if interface_type not in [ network_model.OVS_VHOSTUSER_INTERFACE_TYPE,
                               network_model.OVS_VHOSTUSER_CLENT_INTERFACE_TYPE ]:
        _set_device_mtu(dev, mtu)
    else:
